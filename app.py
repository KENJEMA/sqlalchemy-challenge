...
def welcome():
    ...
    return(
        ...
        "<br/>"
        "-List of stations from dataset<br/>"
        "/api/v1.0/stations<br/>"
        "<br/>"
        "-Temperature observations(TOBS) for previous year<br/>"
        "/api/v1.0/tobs<br/>"
        "<br/>"
        "-Min, Max. and Avg. temperatures for all dates from start date: ('yyyy-mm-dd' date format):<br/>"
        "/api/v1.0/start<br/>"
        "<br/>"
        "-Min. Max. and Avg. tempratures for  start to end date: ('yyyy-mm-dd'/'yyyy-mm-dd' date format):<br/>"
...
def precipitation():
    ...
    # Close session
    session.close()
    # Calculate the annual last date 
    last_data_point = session.query(Measurement.date).order_by(Measurement.date.desc()).first()

    last_date = last_data_point.date

    # Convert last date from string to date
...
def tobs_start(start_date):
    """When given the start DATE only, calculate TMIN, TAVG, and TMAX for all dates ≥ to the start date"""
    # Create our session (link) from Python to the DB
    session = Session(engine)
    # Close session
    session.close()
...
def tobs_start_end(start_date, end_date):
    """When given the start and the end date, calculate the TMIN, TAVG, and TMAX for dates between  start and end date."""
    # Create our session (link) from Python to the DB
    session = Session(engine)
    # Close session
    session.close()
